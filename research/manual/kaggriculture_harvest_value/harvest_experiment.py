...
from harvest_features import clock
START, END = (480, 718)
...
def actors(game, mode, seat, arm):
    ...
    candidate = HarvestPolicy(game, mode, arm)
    reference = HarvestPolicy(game, 'control', arm)
    null = HarvestPolicy(game, 'null', arm)
...
def rollout(payload, key, mode, game, make_environment, expected, emit, progress, actor_factory=actors):
    if key['arm'] != 'coordinated' or mode not in ('control', 'marginal'):
...
def paired(control, candidate):
    ...
    if control['mode'] != 'control' or candidate['mode'] != 'marginal':
