...
def write(p,x):atomic(p,json.dumps(x,sort_keys=True,indent=2,allow_nan=False).encode()+b'\n')
def code_hash(root):
    root=Path(root)
...
def save_cache(p,fp,payload):
    p=Path(p)
    if p.exists():raise ValueError('Refuse to overwrite completed checkpoint')
    doc={'fingerprint':fp,'payload':payload};doc['checksum']=digest(doc)
    atomic(p,gzip.compress(canonical(doc),mtime=0))
    if load_cache(p,fp)!=payload:raise ValueError('Checkpoint readback mismatch')
def load_cache(p,fp):
    p=Path(p)
    if not p.exists():return None
    doc=json.loads(gzip.decompress(p.read_bytes()));checksum=doc.pop('checksum')
    if checksum!=digest(doc) or doc['fingerprint']!=fp:raise ValueError('Checkpoint integrity/lineage mismatch; preserve it')
    return doc['payload']
