...
from maintenance_io import read, sha, inside, write, atomic, digest, code_hash, save_cache, load_cache, utc
...
def verify_package():
    for name, h in read(BASE / 'PACKAGE_MANIFEST.json')['files'].items():
        if sha(inside(BASE, name)) != h:
            raise ValueError('Package file changed: ' + name)

def prior_check(home):
    previous = home / 'kaggriculture_route_opportunity'
    review = read(BASE / 'reference/input_review.json')
    for stage in ('screen', 'pilot'):
        p = previous / 'outputs' / stage / 'report.json'
        r = read(p)
        s = read(p.with_name('status.json'))
        if sha(p) != review[stage + '_report_sha256'] or s.get('status') != 'COMPLETED' or s.get('report_sha256') != sha(p):
            raise ValueError('Notebook11 completed report differs')
        if r['code_sha256'] != code_hash(previous):
            raise ValueError('Notebook11 code differs')
        for rel, h in r['output_sha256'].items():
            if sha(inside(previous / 'outputs', rel)) != h:
                raise ValueError('Notebook11 result changed: ' + rel)
    if r['decision'] != 'STOP_NEGATIVE_ENDPOINT':
        raise ValueError('Different previous research decision')
    for folder, items in read(BASE / 'reference/dependency_hashes.json').items():
        for rel, h in items.items():
            if sha(inside(home / folder, rel)) != h:
                raise ValueError('Dependency changed: ' + folder + '/' + rel)
    return r
...
def preflight(home):
    ...
    fp = digest({'code': code_hash(BASE), 'protocol': sha(BASE / 'PROTOCOL.json'), 'engine': ENGINE, 'previous': sha(home / 'kaggriculture_route_opportunity/outputs/pilot/report.json'), 'commit': COMMIT, 'source_progress': sha(root / 'reports/staffing_progress.json')})
...
def samples():
    return sorted(set(range(0, 696, 6)) | set(range(480, 528)))
...
def screen(home):
    ...
    from mechanics import verify_mechanics
    from water_features import extract, FIELDS, SUMMARY_FIELDS, clock
    from maintenance_policy import MaintenancePolicy
    from maintenance_experiment import measure, index_records
    mechanics = verify_mechanics(game)
    write(OUT / 'mechanics.json', {'status': 'PASSED', 'fixtures': len(mechanics), 'official_component_branches': 2 * len(mechanics), 'cases': mechanics})
    log('MECHANICS_PASSED', fixtures=len(mechanics))
    ...
    for job in jobs:
        ...
        if cached is None:
            ...
            actor = MaintenancePolicy(game, 'defer')
            reference = MaintenancePolicy(game, 'control')
            null = MaintenancePolicy(game, 'null')
            states = []
            plants = []
            for step in samples():
                ...
                changed = actual != ref
                states.append({**key, 'step': step, 'day': step // 24, 'hour': step % 24, 'candidate_callback_ms': ms, 'reference_callback_ms': rms, 'null_callback_ms': nms, 'action_changed_in_window': changed and in_window, 'farm_changed': (actual['farmer'], actual['hands']) != (ref['farmer'], ref['hands']), 'reference_action_parity': True, 'null_action_parity': True, **summary})
    ...
    save_csv(OUT / 'screen/crop_features.csv.gz', plants)
    registry = []
    for field in FIELDS:
    ...
    report = {**common(fp), 'status': 'WATER_MAINTENANCE_SCREEN_COMPLETE', 'tests': tests, 'decision': 'ELIGIBLE_FOR_ONE_PAIRED_PILOT' if chosen else 'STOP_NO_ACTION_ACTIVATION', 'selected_key': chosen, 'observations': len(states), 'source_episodes': 3, 'independent_seed_blocks': 2, 'crop_rows': len(plants), 'plant_descriptors': len(FIELDS), 'state_descriptors': len(SUMMARY_FIELDS), 'official_mechanics_fixtures': len(mechanics), 'action_changes': sum((r['action_changed_in_window'] for r in states)), 'null_action_checks': len(states), 'source_action_checks': len(states), 'new_episode_transitions': 0, 'candidate_callback_max_ms': max((r['candidate_callback_ms'] for r in states)), 'elapsed_seconds': time.monotonic() - started, 'output_sha256': {n: sha(OUT / n) for n in ('mechanics.json', 'screen/states.csv', 'screen/crop_features.csv.gz', 'screen/feature_registry.csv')}, 'limitations': ['Known development sources; turns and both seats are not independent samples.', 'Next-refresh scenario holds other interventions fixed; postponement can overload tomorrow.', 'Live intervention only on zero-indexed days 20 and 21; other days are coverage observations.', 'No official leaderboard score; inherited <=3-hands callback domain remains.']}
...
def pilot(home):
    ...
    if screen_report['selected_key'] is None:
        report = {**common(fp), 'status': 'WATER_MAINTENANCE_PILOT_COMPLETE', 'decision': 'STOP_NO_ACTION_ACTIVATION', 'completed_pairs': 0, 'prefix_replay_transitions': 0, 'suffix_transitions': 0, 'output_sha256': {}, 'elapsed_seconds': time.monotonic() - started}
        write(OUT / 'pilot/report.json', report)
        log(report['status'], decision=report['decision'])
        return
    from maintenance_experiment import rollout, paired, decision
    import pandas as pd
    progress, jobs = source_jobs(root)
...
def supervise(stage, home):
    ...
    child = None
    try:
        child = subprocess.Popen([sys.executable, str(BASE / 'run_maintenance.py'), '_' + stage, '--home', str(home)], start_new_session=True)
        tick = time.monotonic()
        beat = tick
...
def bundle():
    ...
    paths += [BASE / n for n in ('12_water_maintenance_feature_ablation.ipynb', 'PROTOCOL.json', 'FEATURE_RESEARCH.md', 'LOCAL_VALIDATION.json', 'reference/input_review.json')]
    ...
    dest = BASE / 'kaggriculture_water_maintenance_results.zip'
