...
from maintenance_experiment import rollout, paired, decision, index_records, measure
from run_maintenance import samples, select_source
...
class ExperimentTests(unittest.TestCase):
    ...
    def test_no_final_day_screen(self):
        self.assertLess(max(samples()), 696)
    ...
    def test_latency_saved_before_reject(self):
        events = []
        with patch('maintenance_experiment.time.perf_counter', side_effect=[0, 1]), self.assertRaises(RuntimeError):
    ...
    def test_partial_cannot_score(self):
        with self.assertRaises((ValueError, KeyError)):
            paired({'key': {}, 'initial_sha256': 'x', 'mode': 'control', 'suffix_transitions': 1}, {'key': {}, 'initial_sha256': 'x', 'mode': 'defer'})
