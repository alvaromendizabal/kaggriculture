...
import math, time
from maintenance_io import digest
from water_features import clock
START, END = (480, 718)
...
def actors(game, mode, seat, arm):
    ...
    candidate = MaintenancePolicy(game, mode, arm)
    reference = MaintenancePolicy(game, 'control', arm)
    null = MaintenancePolicy(game, 'null', arm)
...
def rollout(payload, key, mode, game, make_environment, expected, emit, progress, actor_factory=actors):
    if key['arm'] != 'coordinated' or mode not in ('control', 'defer'):
...
def paired(control, candidate):
    ...
    if control['mode'] != 'control' or candidate['mode'] != 'defer':
        raise ValueError('Wrong paired modes')
    for branch in (control, candidate):
        if branch['suffix_transitions'] != 239 or branch['null_action_checks'] != 239:
            raise ValueError('Incomplete branch; no endpoint score')
    if control['control_source_transition_checks'] != 238:
        raise ValueError('Control source not reproduced')
    out = dict(control['key'])
