...
BASE=Path(__file__).resolve().parent;OUT=BASE/'outputs';PROCESS_STARTED=time.monotonic()

def log(event,**data):
    OUT.mkdir(exist_ok=True);r={'utc':utc(),'event':event,'process_elapsed_seconds':round(time.monotonic()-PROCESS_STARTED,3),**data}
    with (OUT/'events.jsonl').open('a') as f:f.write(json.dumps(r,allow_nan=False)+'\n');f.flush()
    write(OUT/'progress.json',r)
...
def preflight(home):
    verify_package();protocol=read(BASE/'PROTOCOL.json');old=home/'kaggriculture_manual_resume';prior=home/'kaggriculture_two_rounds'
    for n,h in read(BASE/'reference/prior_result_hashes.json').items():
        if sha(inside(prior,n))!=h:raise ValueError('Prior rounds17/18 evidence changed: '+n)
    ...
    if head!=protocol['source_commit']:
        try:git('merge-base','--is-ancestor',protocol['source_commit'],head)
        except subprocess.CalledProcessError as error:raise ValueError('HEAD is not a descendant of the reviewed baseline; do not reset') from error
    if git('diff',protocol['source_commit'],'--','src','configs','pyproject.toml','uv.lock'):raise ValueError('Core source/config changed; do not reset')
    ...
    seed_audit={'proposed_seeds':sorted(seeds),'repository_text_files_scanned':scanned,'collisions':hits,
      'known_prior_manual_seed_groups':[1601,1602],'reserved_but_unexecuted_in_returned_rounds17_18':[2026091217,2026091218],'scope':'repository textual inventory plus restored/returned known evidence; cannot certify unrecorded activity'}
    ...
    write(OUT/'preflight.json',{'status':'PASSED','utc':utc(),'fingerprint':fp,'source_commit':protocol['source_commit'],'observed_local_head':head,'remote_head_verified':False,'engine':manifest,
...
def screen(rid,home):
    ...
    module=importlib.import_module('collection_features' if rid=='19' else 'maintenance_features')
    folder=OUT/f'round{rid}'/'screen';folder.mkdir(parents=True,exist_ok=True)
    checks=validate(env.game,rid,env.make_environment);write(folder/'mechanics.json',checks);log('MECHANICS_PASSED',round=rid,cases=checks['cases'])
    ...
    for idx,j in enumerate(jobs):
        ...
        if data is None:
            ...
            for rec in p['records']:
                ...
                rr.append({**key,'step':obs['step'],'day':obs['day'],'changed':int(a!=ref),'farm_changed':int((a['farmer'],a['hands'])!=(ref['farmer'],ref['hands'])),'market_changed':int(a['market']!=ref['market']),'candidate_ms':ms,'reference_ms':rms,**sv})
                tt.extend([{**key,'step':obs['step'],**r} for r in tr])
                if len(rr)%48==0:log('SCREEN_PROGRESS',round=rid,source=idx+1,completed=len(rr),source_total=len(wanted),step=obs['step'])
    ...
    report={**common(fp,rid),'status':'FEATURE_SCREEN_COMPLETE','tests':tests,'observations':len(rows),'task_rows':len(tasks),
      ...
      'decision':'REVIEW_REQUIRED_BEFORE_GAMES' if changes else 'STOP_NO_ACTION_ACTIVATION','new_games':0,'elapsed_seconds':time.monotonic()-started,
...
def summary(rid):
    sr=verify_stage(rid,'screen');reports=[]
    for i in range(1,5):
        if (OUT/f'round{rid}'/f'pair{i}'/'status.json').exists():reports.append(verify_stage(rid,f'pair{i}'))
    rows=[r['comparison'] for r in reports if 'comparison' in r]
    csv(OUT/f'round{rid}'/'comparisons.csv',rows)
    decision=('STOP_NO_ACTION_ACTIVATION' if not sr['action_changes'] else 'REVIEW_REQUIRED_BEFORE_GAMES') if not rows else 'REVIEW_COMPLETED_DEVELOPMENT_PAIRS_NO_PROMOTION'
    report={'status':'ROUND_REPORT_SAVED','round':rid,'screen_observations':sr['observations'],
        'screen_action_changes':sr['action_changes'],'decision':decision,'comparisons':rows,
        'completed_pairs':len(rows),'planned_future_pairs':4,'automatic_promotion':False,
        'development_seed_groups':len({r['seed'] for r in rows}),'official_submission_score':None,
        'official_metric_effect_measured':False,'limitations':['Archived repeated-development screen is not a generalization test.',
        'No outcome inference from activation or feature counts.','Proposed future blocks are balanced, not fully crossed.',
        'The complete callback still admits at most three hired hands per farm.','No current account or leaderboard status is certified.']}
    write(OUT/f'round{rid}'/'summary.json',report);log('ROUND_REPORT_SAVED',round=rid,decision=decision)
...
def supervise(rid,stage,home):
    ...
    try:
        child=subprocess.Popen([sys.executable,str(BASE/'run_service.py'),'_worker','--round',rid,'--stage',stage,'--home',str(home)],start_new_session=True)
        start=last=time.monotonic();cap=read(BASE/'PROTOCOL.json')['caps_seconds']['screen' if stage=='screen' else 'pair']
        while child.poll() is None:
            now=time.monotonic()
            if now-start>cap:raise TimeoutError(f'{cap}-second stage cap')
            rss=0
            ps=Path(f'/proc/{child.pid}/status')
            try:process_status=ps.read_text()
            except FileNotFoundError:process_status=''
            if process_status:
                for line in process_status.splitlines():
                    if line.startswith('VmRSS:'):rss=int(line.split()[1])/1024
            if rss>read(BASE/'PROTOCOL.json')['memory_rss_limit_mib']:raise MemoryError('Worker RSS cap exceeded')
            if now-last>=10:
                progress=read(OUT/'progress.json') if (OUT/'progress.json').exists() else {}
                print(json.dumps({'utc':utc(),'event':'HEARTBEAT','round':rid,'stage':stage,'stage_elapsed_seconds':round(now-start,1),'supervisor_elapsed_seconds':round(now-PROCESS_STARTED,1),'worker_rss_mib':round(rss,1),'last_completed_work':progress}),flush=True);last=now
            time.sleep(.2)
...
def bundle():
    paths=[p for p in OUT.rglob('*') if p.is_file() and not p.is_symlink() and p.suffix in ('.json','.jsonl','.csv','.gz','.txt','.html','.log')]
    paths += [p for p in BASE.glob('*.ipynb')]+[BASE/'PROTOCOL.json',BASE/'PREPARATION_STATUS.json',BASE/'reference/input_review.json']
    ...
    dest=BASE/'kaggriculture_service_value_results.zip';fd,tmp=tempfile.mkstemp(prefix='.bundle-',dir=BASE);os.close(fd)
    try:
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('command',choices=['screen','pair','summary','tests','bundle','_worker']);ap.add_argument('--round',choices=['19','20'],default='19');ap.add_argument('--block',type=int,choices=range(1,5),default=1);ap.add_argument('--stage');ap.add_argument('--reviewed-screen');ap.add_argument('--home',type=Path,default=Path.home());a=ap.parse_args()
    try:
        if a.command=='bundle':bundle()
            ...
            if a.command=='pair':
                report_path=OUT/f'round{a.round}'/'screen/report.json'
                sr=verify_stage(a.round,'screen')
                if sr['decision']!='REVIEW_REQUIRED_BEFORE_GAMES':raise RuntimeError('No activated screen permits a pair')
                if a.reviewed_screen!=sha(report_path):raise RuntimeError('Manual review required: supply the exact screen report SHA256 only after discussing this screen')
            supervise(a.round,'screen' if a.command=='screen' else f'pair{a.block}',a.home)
