...
from lifecycle_experiment import rollout, paired, decision, index_records, measure
from run_lifecycle import samples, select_source
...
class ExperimentTests(unittest.TestCase):
    ...
    def test_sample_count(self):
        self.assertEqual(len(samples()), 124)

    def test_all_window_hours_sampled(self):
        self.assertTrue(set(range(192,672,4)).issubset(samples()))
    ...
    def test_latency_saved_before_reject(self):
        events = []
        with patch('lifecycle_experiment.time.perf_counter', side_effect=[0, 1]), self.assertRaises(RuntimeError):
    ...
    def test_live_prefix_and_linked_suffix_toy(self):
        for seat in (0, 1):
            ...
            row = paired(*results)
            self.assertEqual(row['coins_delta'], 3)
            self.assertEqual(results[0]['prefix_replay_transitions'], 192)
            self.assertEqual(results[1]['trace'][-1]['own_cash'], 3003)
    ...
    def test_partial_cannot_score(self):
        with self.assertRaises((ValueError, KeyError)):
            paired({'key': {}, 'initial_sha256': 'x', 'mode': 'control', 'suffix_transitions': 1}, {'key': {}, 'initial_sha256': 'x', 'mode': 'renew'})
