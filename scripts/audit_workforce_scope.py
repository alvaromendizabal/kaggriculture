...
Raw framework states may omit seat one's step. The fixture derives it from legal
public day/hour before isolating the separate workforce-coverage boundary.
"""
...
def probe() -> dict:
    ...
    for hiring_player in (0, 1):
        ...
        for viewer in (0, 1):
            ...
            raw_step_present = "step" in observation
            raw_step = observation.get("step")
            derived_step = int(observation["day"]) * 24 + int(observation["hour"])
            if raw_step is not None and int(raw_step) != derived_step:
                raise ValueError("Raw fixture state clock conflicts with public day/hour")
            # Copy only: do not mutate the engine or any frozen project extractor.
            observation["step"] = derived_step
            detail = {
                "viewer": viewer,
                "raw_step_present": raw_step_present,
                "raw_step": raw_step,
                "fixture_step": derived_step,
            }
            try:
                project_observation(observation)
            except ValueError as error:
                rejections.append({**detail, "rejected": True, "reason": str(error)})
            else:
                rejections.append({**detail, "rejected": False, "reason": None})
    ...
    return {
        ...
        "fixture_clock_adapter": "Copy raw state; derive step from public day/hour",
