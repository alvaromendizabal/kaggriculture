...
def replay(payload: dict) -> dict:
    ...
    policy, causal, history = LivestockPolicy(arm), CausalHistory(), MarketHistory()
    ...
    for step in range(719):
        ...
        if step in samples:
            ...
            if schema != payload["feature_schema"] or len(values) != 1500:
        ...
        if step % 24 == 0 or step == 718:
            policy = LivestockPolicy.from_state_dict(policy.state_dict())
...
def main() -> None:
    ...
    for job in jobs:
        ...
        cached_path = root / "artifacts/livestock_audits" / (digest(lineage) + ".json.gz")
        result = load_checkpoint(cached_path, lineage)
        if result is None:
            if new >= args.max_new_games:
                break
            with progress.stage("audit_livestock_" + "_".join(map(str, job["key"].values()))):
    ...
    pd.DataFrame(rows).to_csv(root / "reports/livestock_action_identities.csv", index=False)
    result = {
    ...
    write_json(root / "reports/livestock_integrity.json", result)
