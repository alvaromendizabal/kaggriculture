"""Register, execute at most eight new games, or summarize the frozen supply study."""
...
from kaggriculture_research.supply_experiment import job_plan, run_batch, summarize


def register(root: Path, protocol: dict) -> dict:
    common, jobs = job_plan(root, protocol)
    path = root / "reports/supply_registration.json"
    identity = {
        "lineage": common,
...
def main() -> None:
    ...
    if args.mode == "register":
        ...
        return
    if not (root / "reports/supply_registration.json").exists():
    ...
    if args.mode == "batch":
        ...
        with progress.stage("supply_summary_and_joint_feature_screen"):
