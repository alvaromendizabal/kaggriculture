...
from kaggriculture_livestock.policy import ARMS, LivestockPolicy
...
OPPONENTS = ("market10", "livestock_routine")
CONTRASTS = (
    ("feed", "routine"),
    ("care", "feed"),
    ("fertilizer", "care"),
...
def feature_bank(obs: dict, past, market_past) -> tuple[dict, dict]:
    ...
    if len(values) != 1500 or len(values) != sum(len(f.values) for f in vectors):
...
def run_game(seed: int, seat: int, opponent: str, arm: str, stride: int = 4) -> dict:
    ...
    candidate = LivestockPolicy(arm)
    rival = MarketPolicy("10") if opponent == "market10" else LivestockPolicy("routine")
        ...
        def call(observation, configuration):
            ...
            if is_candidate:
                ...
                if obs["step"] % 24 == 0 or obs["step"] == 718:
                    candidate = LivestockPolicy.from_state_dict(candidate.state_dict())
...
def validate_episode(payload: dict, key: dict) -> None:
    ...
    if (
        len(payload["feature_columns"]) != 1500
        or len(set(payload["feature_columns"])) != 1500
    ...
    for sample in payload["feature_samples"]:
        if len(sample["values"]) != 1500 or not np.isfinite(sample["values"]).all():
...
def common_lineage(root: Path, protocol: dict) -> dict:
    ...
    expected = {
        ...
        "bootstrap_seed": 4401,
        "work_opportunity_coins": 8.0,
        "plant_scenario_horizon": 168,
    }
    if (
        any(protocol.get(k) != v for k, v in expected.items())
        or set(protocol["development_seeds"]) & forbidden
    ):
        raise ValueError("Registered livestock design or seed separation changed")
    frozen = json.loads((root / "reports/supply_registration.json").read_text())["identity"][
        "lineage"
    ]["code"]
    for name, sha in frozen.items():
        if file_digest(root / "src/kaggriculture_research" / name) != sha:
            raise ValueError("Frozen prior source changed: " + name)
    files = [
        ...
        jobs.append(
            {
                "key": key,
                "lineage": lineage,
                "path": "artifacts/livestock_episodes/" + digest(lineage) + ".json.gz",
            }
        )
    return common, jobs
...
def run_batch(
    ...
    write_json(root / "reports/livestock_progress.json", status)
    return status
