"""Fixed mixed-farm scaffold for sequential feed, care and fertilizer ablations.
...
class LivestockPolicy:
    ...
    def __call__(self, observation: dict) -> dict:
        ...
        reserve_feed = 2 * active if obs["day"] < 29 else sum(not t["fed_today"] for t in animals)
        reserve_manure = 0
    ...
    @classmethod
    def from_state_dict(cls, state: dict) -> "LivestockPolicy":
