"""Frozen 2x2x2 development design, causal feature coverage and conservation."""
...
from kaggriculture_research.relationship_policy import ARMS, FACTORS, RelationshipPolicy


def run_game(seed: int, seat: int, arm: str, stride: int = 4) -> dict:
    candidate, opponent = RelationshipPolicy(arm), CropPolicy("full", 3)
...
def factorial_summary(frame: pd.DataFrame, resamples: int, bootstrap_seed: int) -> pd.DataFrame:
    """Main effects, averaged pairwise difference-in-differences and triple contrast.
    ...
    Bootstrap intervals are descriptive; four clusters cannot establish generality.
    """
...
def screen_features(root: Path, matrix: pd.DataFrame, metadata: pd.DataFrame, schema: dict) -> dict:
    ...
    pd.DataFrame(correlated, columns=["first", "second", "spearman"]).to_csv(
        root / "reports/relationship_correlations.csv", index=False
    )
    registry = []
    for name in matrix:
        registry.append(
    ...
    pd.DataFrame(registry).to_csv(root / "reports/relationship_registry.csv", index=False)
    ...
    path = root / "artifacts/relationship_features" / (digest(lineage) + ".json.gz")
    save_checkpoint(
        path,
        lineage,
        {
...
def run_experiment(
    ...
    if (
        protocol["development_seeds"] != foundation["development_seeds"]
    ...
    if file_digest(source / "crop_policy.py") != protocol["frozen_crop_policy_sha256"]:
    ...
    for seed, seat, arm in jobs:
        ...
        path = root / "artifacts/relationship_episodes" / (digest(lineage) + ".json.gz")
        with progress.stage(f"relationships_{seed}_seat{seat}_arm{arm}"):
    ...
    frame.to_csv(root / "reports/relationship_games.csv", index=False)
    effects = factorial_summary(frame, protocol["bootstrap_resamples"], protocol["bootstrap_seed"])
    effects.to_csv(root / "reports/relationship_effects.csv", index=False)
    ...
    report = {
        ...
        "limitations": [
            "Four reused development seeds; no confirmatory inference or multiple-testing claim",
            "One stronger frozen opponent this batch; previous study used two weaker opponents",
            "Only three policy components activated; other feature families remain provisional",
            "Same initial seed does not fix future shops after policy-induced RNG divergence",
            "Refresh counterfactuals exclude preceding decay and assume treatment availability",
            "Early cap collection is local; routing and all opportunity costs are not optimized",
        ],
    }
    write_json(root / "reports/relationship_research.json", report)
    return report
